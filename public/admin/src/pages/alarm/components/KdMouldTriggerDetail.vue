...
<!-- @description File path and name:   kundian_iot_admin/src/pages/alarm/components/KdMouldTriggerDetail -->
...
<template>
<div v-if="state.show">
	<a-modal title="模板触发器信息" v-model:visible="state.show" width="600px" :footer="null">
			...
			<div class="info-box">
				<div v-if="state.info" class="info-box">
					...
					<div class="info-item flex">
						...
						<div class="ii-value">
							{{ state.info?.template?.name || ''}} - 
							{{ state.info?.templateSubordinate?.name || ''}} - 
							{{ state.info?.templateSubordinateVariable?.name || ''}}
						</div>
					</div>
					<div class="info-item flex">
					...
					<div class="info-item flex">
						...
						<div class="ii-value"><a-switch disabled v-model="state.info.is_alarm" :checked-value="1" :unchecked-value="0"></a-switch></div>
					</div>
					...
					<template v-if="state.info.is_linkage ===1">
						<div class="info-item flex">
							<div class="ii-title">联动类型：</div>
							<div class="ii-value">
								<span>{{ state.info.linkage_type ===1 ?'采集':'控制'}}</span>
							</div>
						</div>
						<div class="info-item flex">
							<div class="ii-title">联动变量：</div>
							<div class="ii-value">
								{{state.info?.linkageSubordinate.name}} - 
...
import { getMouldTriggerDetail } from '@/api/kdTrigger'
...
function show(id){
	state.show = true
	state.loading = true
	getMouldTriggerDetail({id}).then(res=>{
		state.loading = false
		state.info = res.data
	}).catch(()=>{
		state.loading = false
