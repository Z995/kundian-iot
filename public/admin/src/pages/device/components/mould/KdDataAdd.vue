...
<!-- @description File path and name:   kundian_iot_admin/pages/device/components/mould/KdDataAdd.vue -->
...
<!-- @time:2025-06-03 10:31:54  -->
<template>
<div v-if="state.show">
	<a-modal :title="state.title" v-model:visible="state.show" width="700px" :on-before-ok="saveData">
...
const state = reactive({
	...
	register_value:'',			//寄存器
	register_address:"",		//寄存器实际地址
	more:false,
	protocol:1,					//从机协和产品类型 1:Modbus RTU 2:自定义协议
})
function show(data,protocol){
	state.show = true
	state.title = data ?'编辑变量':'新增变量'
	state.protocol = protocol ||1
	if( data ){
		state.form = {
			...data
		}
