...
namespace app\queue\redis\variable;
...
class Log implements Consumer
{
    ...
    {
        $this->queue = 'iots_redis_queue_variable_log';
    }
    public function consume($data)
    {
        try {
            (new DeviceSubordinateVariableLogServices())->saveLog($data["gateway_id"],$data["code"],
                $data["subordinate_address"],  $data["function_code"],
                $data["variable_id"],$data["value"]);
        } catch (\Throwable $e) {
            SysLog::channel($this->logFile)->info('variable-log异常: ' . $e->getMessage(), [
                'package' => $data,
                'file' => $e->getFile(),
                'line' => $e->getLine(),
