...
class AlarmTemplateTriggerServices extends BaseServices
{
    ...
    public function getRedisTemplateTrigger($template_subordinate_variable_id)
    {
        $key= "AlarmTemplateTrigger:" . $template_subordinate_variable_id;
        ...
        if (empty($template)){
            ...
            Redis::set($key,json_encode($template),"Ex", $expire);
        }else{
...
//        $template=$this->getSelect(["template_subordinate_variable_id"=>$template_subordinate_variable_id]);
        $template=$this->getRedisTemplateTrigger($template_subordinate_variable_id);
